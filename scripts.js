...
(function () {
  ...
  */
  ...
  /* ══════════════════════════════════════════════════════════════════
     VISION FORGE — HAMBURGER PANEL & SPA NAVIGATION
     Moved from index.html inline script. Capture-phase handler here
     runs before any bubble-phase listeners in this same IIFE.
  ══════════════════════════════════════════════════════════════════ */

'use strict';

  /* ═══════════════════════════════════════════════════════
     VISION FORGE — HAMBURGER PANEL (fully self-contained)
     All scripts.js conflicts resolved:
       1. Hamburger & overlay cloned on init  → strips scripts.js listeners
       2. aggressiveUnlock polls every rAF    → overrides any re-lock
       3. Capture-phase nav handler           → runs before scripts.js
       4. Single source of truth for state    → no double-register
  ═══════════════════════════════════════════════════════ */
  ...
  /* Hard clear — used both immediately and in the unlock poller */
  function forceUnlock() {
    document.body.classList.remove('panel-open');
    document.body.style.overflow   = '';
    document.documentElement.style.overflow = '';
    /* Also clear any position-based scroll-lock scripts.js might use */
  ...
  /*
   * Polls forceUnlock on every animation frame for ~600 ms.
   * This beats any setTimeout or rAF-based re-lock that scripts.js
   * might fire after panel close.
   */
  var _unlockTimer = null;
  function aggressiveUnlock() {
    forceUnlock();
    var frames = 0;
    var MAX    = 36; /* ~600 ms @ 60 fps */
    function tick() {
      forceUnlock();
      frames++;
      if (frames < MAX) _unlockTimer = requestAnimationFrame(tick);
    }
    if (_unlockTimer) cancelAnimationFrame(_unlockTimer);
    _unlockTimer = requestAnimationFrame(tick);
  }

  /* ── 2. PANEL STATE ─────────────────────────────────── */
  var btn, panel, overlay, closeBtn;
  var panelReady = false;

  /*
   * Clone both the hamburger button and the overlay to strip any
   * event listeners that scripts.js may have attached before this
   * inline script runs (scripts.js uses defer, so it fires first).
   */
  function stripExternalListeners() {
    var oldBtn = document.getElementById('hamburgerBtn');
    if (oldBtn) {
      var newBtn = oldBtn.cloneNode(true);
      oldBtn.parentNode.replaceChild(newBtn, oldBtn);
    }
    var oldOv = document.getElementById('spOverlay');
    if (oldOv) {
      var newOv = oldOv.cloneNode(true);
      oldOv.parentNode.replaceChild(newOv, oldOv);
    }
  }

  function initPanel() {
    /* Strip scripts.js listeners first */
    stripExternalListeners();

    btn      = document.getElementById('hamburgerBtn');
    panel    = document.getElementById('slidePanel');
    overlay  = document.getElementById('spOverlay');
    closeBtn = document.getElementById('spCloseBtn');
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && panelReady && panel.classList.contains('is-open')) {
        closePanel();
      }
    });
  }
  ...
  function closePanel() {
    ...
    /* Use aggressive unlock so scripts.js can never re-lock the page */
    aggressiveUnlock();
  }
  ...
  function navigateTo(pageId, anchor) {
    ...
    /*
     * scrollTo must run AFTER aggressiveUnlock restores overflow,
     * so we defer it one frame.
     */
  ...
  /* ── 4. LOGO DARK / LIGHT SWITCHER ─────────────────── */
  var DARK_PAGES = ['team', 'explore'];
  function updateLogo() {
    var active = document.querySelector('.page-view.page-active');
    if (!active) return;
    var pageId = active.id.replace('page-', '');
    document.body.classList.toggle('dark-hero-page', DARK_PAGES.indexOf(pageId) > -1);
  }
  new MutationObserver(updateLogo).observe(document.body, {
    subtree: true, attributeFilter: ['class']
  });

  /* ── 5. CAPTURE-PHASE CLICK HANDLER ────────────────── */
  /*
   * Runs before any scripts.js bubble-phase handler.
   * stopImmediatePropagation prevents scripts.js from seeing the
   * click at all, so it cannot re-lock scroll or double-navigate.
   */
  document.addEventListener('click', function (e) {
    var el = e.target.closest('[data-page]');
    if (!el) return;

    /* Let real file links and external URLs pass through */
    var href = el.getAttribute('href') || '';
    ...
    var wasOpen = panelReady && panel && panel.classList.contains('is-open');
    ...
    /*
     * If we just closed the panel, wait one frame so closePanel's
     * aggressiveUnlock has already fired before we switch pages
     * (avoids the "scroll stuck at 0" flash on slow devices).
     */
    if (wasOpen) {
      requestAnimationFrame(function () {
  ...
  window.vfNavigate   = navigateTo;
  window.vfClosePanel = closePanel;
})();
